...
const EditModal: React.FC<Props> = ({ transaction, isOpen, onClose, onUpdate, loading }) => {
  ...
  const handleSubmit = async () => {
    ...
    // If it's a repair, we validate price and account before marking done
    ...
    if (!success) {
      setError('Incorrect PIN or Update Failed');
    } else {
      onClose();
    }
  };
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={isRepair ? "Complete Repair Job" : "Mark as Paid"}>
      <div className="space-y-4">
        ...
        {isRepair && (
          <>
            <div className="p-3 bg-blue-50 text-blue-800 text-sm rounded-lg mb-2">
               Closing job for: <strong>{transaction.customer_name}</strong> ({transaction.phone_name})
            </div>
            <div>
        ...
        {!isRepair && (
          <p className="text-gray-600 text-sm p-2 bg-gray-50 rounded-lg">
            Confirm marking <strong>{transaction.type.toUpperCase()}</strong> of <strong>LKR {transaction.amount}</strong> as paid?
          </p>
        )}
