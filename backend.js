...
function doPost(e) {
  try {
    ...
    if (action === "addTransaction") {
      ...
      // STRICT PIN CHECK ON SERVER SIDE
...
function handleUpdateTransaction(userId, txId, updates) {
  ...
  for (let i = 1; i < data.length; i++) {
    if (String(data[i][colMap['id']]) === String(txId)) {
  ...
  if (!found) {
     return ContentService.createTextOutput(JSON.stringify({ success: false, message: "Transaction not found" })).setMimeType(ContentService.MimeType.JSON);
  }
